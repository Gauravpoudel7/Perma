...
| Row action | "Close" (open short, or an open long — including one owing less than the settle floor) · "Settle" (an open long whose estimated owed premium is at least **0.001 USDC** — routes to `settle_premium`, not burn). `settle_premium` leaves the long open, so it accrues again immediately; the floor stops a 0.000001 USDC tail from re-offering the button as though the settle had failed. Close always settles whatever is owed, at burn. |
| Row action, Pending Premium short | No "Close". `burn_position` requires Open and returns `PositionAlreadyClosed` once liquidity is already out. "Settle" when the range escrow can pay a non-zero amount (`settle_premium`; the 0.001 USDC floor does not apply — this short does not re-accrue, and any payment reduces the claim). If that payment clears `premium_receivable`, the account closes. Caption: "Pays this claim and closes the account." If the escrow holds less than the claim: "Range escrow can pay {n} USDC now. The rest stays owed." If the escrow is empty, or the range account has not loaded: "There is nothing to settle yet. Waiting for a long to fund the range escrow." (`NothingToSettle`, then why.) Est. accrued stays visible. |
...
| Detail sheet, Pending Premium | Sub-line "Liquidity is already withdrawn. Settle pays this claim from the range escrow when it has USDC." Footer is the pending-premium row action above, not Close. |
...
| Position sheet | Title "Position" · sub-line = the premium column note above, or the Pending Premium sub-line when that is the status · rows "Side" · "Realized range" (hint "ticks {l} to {u}") · "Position size" ("{n} liquidity units") · "Status" · "Accrued premium" ("Est. {n} USDC") · "Position account" (truncated address, "Copy" → "Copied", "View account") · link "Open a similar position on Trade" (prefills side + range on the ticket) · footer = Close / Settle, or the Pending Premium action |
